...
EVENTRESULT ppTestAnalysis::RunEvent (){
  ...
  for ( int i=0 ; i<pFullEvent->GetEntries() ; ++i ){
    ...
    if ( sv->Pt()< pars.PtConsMin || sv->Pt()> pars.PtConsMax )             continue;
  ...
  // ue
  vector<double> ptue_arr{};
  if ( JAResult.size() != 0 && particles.size() != 0) {
	for (PseudoJet &part: particles) {
		if (part.user_info<JetAnalysisUserInfo>().GetQuarkCharge() == 0)	continue;
		PseudoJet &lead = JAResult[0];
		if (abs(lead.delta_phi_to(part)) < pi/3 || abs(lead.delta_phi_to(part)) > 2*pi/3)	continue;
		if (part.perp() > 0.2) {
			ptue_arr.push_back(part.perp());
		}
	}
  }
  ...
  for (unsigned ijet = 0; ijet < JAResult.size(); ijet++) {
    ...
    // ue
    int nue = ptue_arr.size();
    double ptue = 0;
    if ( nue != 0 ) {
	ptue = accumulate(ptue_arr.begin(), ptue_arr.end(), 0.0) / nue;
    }

    if ( pars.MaxJetNEF<1.0 && ptne/pttot > pars.MaxJetNEF ) continue;
    ...
    Result.push_back ( ResultStruct ( CurrentJet, SDJet, nch, q, nue, ptue, reject) );
